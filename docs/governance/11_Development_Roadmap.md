...
- [ ] C12. Low-stock alerts — real-time, deterministic (PD-009, PR-9). Reuses C10's `evaluate_low_stock` rule as-is; this stage adds the trigger (fire on every inventory-changing write, not on-demand), per-product/category configurable thresholds (PR-9.3 — no config storage exists yet), and persistence to the existing `alerts` table.
