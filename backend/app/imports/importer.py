...
def run_import(db: Session, upload: Upload, import_record: ImportRecord) -> ImportResult:
    ...
    try:
        ...
        # Resolves an open freshness notification the moment new sales
        # data actually lands, rather than waiting for the next scheduler
        # tick (up to 15 minutes) to notice — no-ops for any other
        # entity_type (app/application/notifications.py::resolve_data_freshness).
        resolve_data_freshness(db, business_id=upload.business_id, entity_type=upload.entity_type)
        db.commit()
    except Exception:
