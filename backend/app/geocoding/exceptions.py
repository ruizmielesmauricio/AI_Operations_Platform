class GeocodingProviderError(Exception):
    ...
    distinct from "nothing matched," which isn't an error at all, just an
    empty result. Caught by app/geocoding/service.py and turned into a
    quiet empty list rather than a raised, unhandled error — the same
    PR-5.4-style posture already used for AI provider failures in
    app/ai/client.py.
    """
